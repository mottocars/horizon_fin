...
export default function GerarContasBancariasModal({ open, onClose, onGerado }) {
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true);
    try {
      const result = await gerarContasBancarias(Number(empresaId));
      onGerado(result);
    } catch (err) {
      setError(err.response?.data?.message || 'Não foi possível gerar as contas bancárias.');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Modal open={open} onClose={onClose} title="Gerar Contas Bancárias">
