...
import { listPlanosGerados } from '../../api/planosFinanceirosSienge.api';
...
export default function PlanosFinanceirosList() {
  ...
  const loadItems = useCallback(async () => {
    setLoading(true);
    try {
      const result = await listPlanosGerados();
      setItems(result);
    } finally {
      setLoading(false);
    }
  ...
  function handleGerado(result) {
    setModalOpen(false);
    loadItems();
    navigate(`/cadastros/planos-financeiros/${result.empresa_id}`);
  }
    ...
    <div className="space-y-4">
      <Card>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-sm font-semibold text-gray-900">Planos financeiros gerados</h2>
            <p className="text-xs text-gray-500">
              Planos de contas importados por empresa a partir dos sistemas integrados.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-2 py-12 text-center text-sm text-gray-400">
            ...
            Nenhum plano financeiro gerado ainda.
          </div>
        ) : (
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {itemsVisiveis.map((item) => (
                <tr
                  key={item.empresa_id}
                  onClick={() => navigate(`/cadastros/planos-financeiros/${item.empresa_id}`)}
