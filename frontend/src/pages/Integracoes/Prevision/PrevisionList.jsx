...
import {
  listPrevisionIntegracoes,
  setPrevisionStatus,
  sincronizarPrevision,
...
export default function PrevisionList() {
  ...
  const loadItems = useCallback(
    async (page, searchTerm, status) => {
      ...
      try {
        ...
        const result = await listPrevisionIntegracoes({ page, limit: LIMIT, search: searchTerm, ativo });
  ...
  async function handleToggleStatus(item) {
    ...
    const confirmado = await confirm({
      ...
      description: `Deseja realmente ${acao} a integração Prevision de "${item.empresa_razao_social}"?`,
    ...
    try {
      await setPrevisionStatus(item.id, novoStatus);
  ...
  async function handleSincronizar(item) {
    ...
    try {
      const result = await sincronizarPrevision(item.empresa_id);
      const linhas = result.detalhes
        .filter((d) => !d.sucesso)
        .map((d) => `• ${d.nome} (${d.primary ? 'primário' : 'alternativo'}): ${d.erro}`);
      await alert({
        title: `Sincronização concluída: ${result.sucesso} de ${result.sucesso + result.falha}`,
        description:
          linhas.join('\n') ||
          `${result.total_centros} centro(s) de custo sincronizado(s) com sucesso.`,
        variant: result.falha > 0 ? 'warning' : 'default',
      });
    } catch (err) {
      await alert({
    ...
    <div className="space-y-4">
      <Card>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button onClick={() => navigate('/integracoes/prevision/nova')}>
            <Plus size={16} />
            Nova Integração
          </Button>
        </div>
                  ...
                  <tr key={item.id} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="py-3">
                      <div className="flex justify-end gap-1">
                        <IconButton
                          title="Sincronizar dados do Prevision"
                          onClick={() => handleSincronizar(item)}
                          disabled={sincronizandoId === item.id || !item.ativo}
                        >
                          <RefreshCw size={16} className={sincronizandoId === item.id ? 'animate-spin' : ''} />
                        </IconButton>
                        <IconButton
                          title="Editar"
                          onClick={() => navigate(`/integracoes/prevision/${item.id}`)}
                        >
                          <Pencil size={16} />
                        </IconButton>
                        <IconButton
