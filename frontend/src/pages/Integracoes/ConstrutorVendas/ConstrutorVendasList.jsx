...
import { listConstrutorVendasIntegracoes, setConstrutorVendasStatus } from '../../../api/construtorVendas.api';
import { formatCnpj } from '../../Empresas/format';
import { useConfirm } from '../../../confirm/ConfirmContext';

const LIMIT = 8;

export default function ConstrutorVendasList() {
  ...
  const loadItems = useCallback(async (page, searchTerm, status) => {
    ...
    try {
      ...
      const result = await listConstrutorVendasIntegracoes({ page, limit: LIMIT, search: searchTerm, ativo });
  ...
  async function handleToggleStatus(item) {
    ...
    const confirmado = await confirm({
      ...
      description: `Deseja realmente ${acao} a integração Construtor de Vendas de "${item.empresa_razao_social}"?`,
    ...
    try {
      await setConstrutorVendasStatus(item.id, novoStatus);
    ...
    <div className="space-y-4">
      <Card>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative w-full sm:max-w-xs">
              ...
              <input
                ...
                placeholder="Buscar por empresa, tenant ou e-mail..."
          ...
          <Button onClick={() => navigate('/integracoes/construtor-de-vendas/nova')}>
            <Plus size={16} />
            Nova Integração
          </Button>
        </div>
                  ...
                  <tr key={item.id} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="py-3">
                      <div className="flex justify-end gap-1">
                        <IconButton
                          title="Editar"
                          onClick={() => navigate(`/integracoes/construtor-de-vendas/${item.id}`)}
                        >
                          <Pencil size={16} />
                        </IconButton>
                        <IconButton
