...
import { listZapiIntegracoes, setZapiStatus } from '../../../api/zapi.api';
import { formatCnpj } from '../../Empresas/format';
import { useConfirm } from '../../../confirm/ConfirmContext';

const LIMIT = 8;

// Mesmo padrão de SiengeList.jsx (lista + busca + filtro de status +
// paginação) — só sem o botão de sincronizar em lote, que é específico do
// Sienge (centros de custo/plano/unidades).
export default function ZapiList() {
  ...
  const loadItems = useCallback(async (page, searchTerm, status) => {
    ...
    try {
      ...
      const result = await listZapiIntegracoes({ page, limit: LIMIT, search: searchTerm, ativo });
  ...
  async function handleToggleStatus(item) {
    ...
    const confirmado = await confirm({
      ...
      description: `Deseja realmente ${acao} a conexão Z-API "${item.nome_conexao}" (${item.empresa_razao_social})?`,
    ...
    try {
      await setZapiStatus(item.id, novoStatus);
    ...
    <div className="space-y-4">
      <Card>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative w-full sm:max-w-xs">
              ...
              <input
                ...
                placeholder="Buscar por empresa, conexão ou instância..."
          ...
          <Button onClick={() => navigate('/integracoes/z-api/nova')}>
            <Plus size={16} />
            Nova Conexão
          </Button>
        </div>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-xs uppercase tracking-wide text-gray-400">
                  ...
                  <th className="py-3 font-medium">ID da instância</th>
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="py-3 font-mono text-xs text-gray-600">{item.instance_id}</td>
                    <td className="py-3">
                      <span
                    ...
                    <td className="py-3">
                      <div className="flex justify-end gap-1">
                        <IconButton title="Editar" onClick={() => navigate(`/integracoes/z-api/${item.id}`)}>
                          <Pencil size={16} />
                        </IconButton>
                        <IconButton
