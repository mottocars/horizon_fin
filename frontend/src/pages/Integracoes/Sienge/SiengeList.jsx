...
import { Plus, Search, Boxes, Pencil, Ban, CheckCircle2, RefreshCw } from 'lucide-react';
import Card from '../../../components/Card';
import Button from '../../../components/Button';
import IconButton from '../../../components/IconButton';
import Pagination from '../../../components/Pagination';
import SearchableSelect from '../../../components/SearchableSelect';
import { listSiengeIntegracoes, setSiengeStatus } from '../../../api/sienge.api';
...
export default function SiengeList() {
  ...
  const loadItems = useCallback(
    async (page, searchTerm, status) => {
      setLoading(true);
      try {
        const ativo = status === 'ativas' ? true : status === 'inativas' ? false : undefined;
        const result = await listSiengeIntegracoes({ page, limit: LIMIT, search: searchTerm, ativo });
  ...
  async function handleToggleStatus(item) {
    ...
    const confirmado = await confirm({
      title: `${novoStatus ? 'Reativar' : 'Desativar'} integração`,
      description: `Deseja realmente ${acao} a integração Sienge de "${item.empresa_razao_social}"?`,
    ...
    try {
      await setSiengeStatus(item.id, novoStatus);
  ...
  async function handleSincronizarTudo(item) {
    setSincronizandoId(item.id);
    const resultados = [];
    let houveFalha = false;

    const etapas = [
      { nome: 'Centros de Custo', executar: () => gerarCentrosCusto(item.empresa_id) },
      { nome: 'Planos Financeiros', executar: () => gerarPlano(item.empresa_id) },
      { nome: 'Mapa de Unidades', executar: () => gerarUnidades(item.empresa_id) },
    ];

    for (const etapa of etapas) {
      try {
        const resultado = await etapa.executar();
        resultados.push(`${etapa.nome}: ${resultado.total_importado} importado(s).`);
      } catch (err) {
        houveFalha = true;
        resultados.push(`${etapa.nome}: falhou (${err.response?.data?.message || 'erro desconhecido'}).`);
      }
    }

    await alert({
      title: `Sincronização de "${item.empresa_razao_social}" concluída`,
      description: resultados.join('\n'),
      variant: houveFalha ? 'warning' : 'default',
    });
    ...
    <div className="space-y-4">
      <Card>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative w-full sm:max-w-xs">
              ...
              <input
                ...
                placeholder="Buscar por empresa ou username..."
          ...
          <Button onClick={() => navigate('/integracoes/sienge/nova')}>
            <Plus size={16} />
            Nova Integração
          </Button>
        </div>
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="py-3 text-gray-600">{item.tenant}</td>
                    ...
                    <td className="py-3">
                      <div className="flex justify-end gap-1">
                        <IconButton
                          title="Atualizar Centros de Custo, Planos Financeiros e Mapa de Unidades"
                          onClick={() => handleSincronizarTudo(item)}
                          disabled={sincronizandoId === item.id || !item.ativo}
                        >
                          <RefreshCw
                            size={16}
                            className={sincronizandoId === item.id ? 'animate-spin' : ''}
                          />
                        </IconButton>
                        <IconButton
                          title="Editar"
                          onClick={() => navigate(`/integracoes/sienge/${item.id}`)}
                        >
                          <Pencil size={16} />
                        </IconButton>
                        <IconButton
