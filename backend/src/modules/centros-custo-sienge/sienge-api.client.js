...
function buildEnterprisesUrl(tenant) {
  return `https://api.sienge.com.br/${tenant}/public/api/v1/enterprises`;
}

async function fetchPage({ tenant, username, password, offset }) {
  const auth = Buffer.from(`${username}:${password}`).toString('base64');
  const url = `${buildEnterprisesUrl(tenant)}?limit=${PAGE_LIMIT}&offset=${offset}`;
...
async function fetchAllEnterprises({ tenant, username, password }) {
  const all = [];
  let offset = 0;
  let total = Infinity;
...
module.exports = { fetchAllEnterprises };
